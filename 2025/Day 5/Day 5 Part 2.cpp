...
int main()
{
    ...
    {
        counter = counter + (range.second - range.first) + 1;

        for(auto j : ranges)
        {
            if(range.first < j.second && range.first > j.first)
            {
                counter = counter - (j.second - range.first + 1);
            }
            ///Answer too High
            else if(range.second < j.second && range.second > j.first)
            {
                counter = counter - (range.second - j.first + 1);
            }
            ///Now answer too Low
        }
    }
